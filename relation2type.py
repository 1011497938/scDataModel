...
open('./temp_data/relation_code2type.json', 'w', encoding='utf-8').write(json.dumps(code2rel, indent=5, ensure_ascii = False) )

data = open('./data/relation_code2type.csv', 'r', encoding='utf-8').read().strip('\n').split('\n')
trigger2score = {}
for row in data[1:]:
	row = row.split(',')
	name = row[0].replace('||',',')
...
open('./data/relation_code2type.json','w', encoding='utf-8').write(json.dumps(trigger2score, indent=4, ensure_ascii = False))


# fs = open('./temp_data/relation_code2type.csv', 'w', encoding='utf-8')
