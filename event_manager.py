...
class EventManager(object):
	...
	def loadPostOfficeEvents(self, LIMIT = 1000000,SKIP = 0, person_id = None):
		...
		if person_id is None:
			query = 'MATCH (person:Biog_main)--(n1:Posted_to_office_data)--(n2:Posting_data)--(n3:Posted_to_addr_data) RETURN person,n1,n2,n3,id(n1),id(n2),id(n3) SKIP {} LIMIT {} '.format(str(SKIP), str(LIMIT))
		else:
			query = 'MATCH (person:Biog_main{{ c_personid:"{}" }})--(n1:Posted_to_office_data)--(n2:Posting_data)--(n3:Posted_to_addr_data) RETURN person,n1,n2,n3,id(n1),id(n2),id(n3) SKIP {} LIMIT {} '.format(str(person_id) ,str(SKIP), str(LIMIT))
		...
		# 加载地点
		results = graph.run('START n=node({}) MATCH (addr:Addr_codes)-[r]-(n) RETURN addr.c_addr_id as addr_id, id(n)'.format(id_list_str3))
		results = results.data()
